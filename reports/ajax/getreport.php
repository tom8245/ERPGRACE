...
if ($_POST["mobile"] != null) {
    $key = $_POST["mobile"];
    $sql .= "and stu_mobile like '%$key%'";
    $sql2 .= "and f_mobile like '%$key%'";
}
if ($_POST["aadhar"] != null) {
    $key = $_POST["aadhar"];
    $sql .= "and stu_aadhar like '%$key%'";
    $sql2 .= "and f_aadhaarno like '%$key%'";
}
if ($_POST["bankno"] != null) {
    $key = $_POST["bankno"];
    $sql .= "and stu_accno like '%$key%'";
}
if ($_POST["hos"] != null) {
    $key = $_POST["hos"];
...
if ($_POST["expfr"] != null || $_POST["expto"] != null) {
    ...
    $sql2 .= "and f_exp between " . $exp1 . " and " . $exp2 . "";
}
...
if (isset($_POST['checkbox'])) {
    ...
    <button onclick="exportpdf()" class="btn-pri">Export to PDF</button>
    <div class="fs-5">ANY DATA REPORT</div>
    <button onclick="exportexcel()" class="btn-pri">Export to Excel</button>
</div>';
    // table
    $html = "<div class='table-responsive mt-2'><table>
<thead>
    <tr>";
    foreach ($cbox as $c):
        $html .= "<td>$c</td>";
...
<tbody>";
    while ($row = mysqli_fetch_assoc($sqlq)) {
        ...
        if (in_array('Mobile Number', $cbox)) {
        ...
        // Bank
        if (in_array('Account Name', $cbox)) {
            $field[] = $row['stu_nameasbank'];
        }
        if (in_array('Account No', $cbox)) {
            $field[] = $row['stu_accno'];
        }
        if (in_array('IFSC code', $cbox)) {
            $field[] = $row['stu_ifsc'];
        }
        if (in_array('Bank Name', $cbox)) {
            $field[] = $row['stu_bankname'];
        }
        if (in_array('Branch Name', $cbox)) {
            $field[] = $row['stu_brancename'];
        }
        if (in_array('Scholarship Status', $cbox)) {
            $field[] = $row['stu_scholarsts'];
        }
        //
        if ($student) {
            $html .= "<tr>";
    ...
    while ($row = mysqli_fetch_assoc($sqlq2)) {
        ...
        // Process Experience Details
        if (in_array('Experience Details', $cbox)) {
            $field2[] = 'Teaching : ' . $row['f_teachexp'] . '<br>Industry : ' . $row['f_indexp'] . '<br>No.of Projects Guiding : ' . $row['f_projguide'];
        }
