...
if (isset($_SESSION['user_id'])) {
  ...
  $sql = "SELECT ce_exam FROM erp_createexam where ce_id=$ExamCEId;";
  $result = mysqli_query($conn, $sql);
  $ExamName = mysqli_fetch_assoc($result)["ce_exam"];
  ...
  <form action="PostPage.php" method="post">
    ...
    if ($numrow > 0) {
      ...
      <input type="hidden" name="resultRecords[' . $i . '][regno]" value="' . $row['stu_id'] . '" ></td>';
        echo '<td>
      <input type="number" name="resultRecords[' . $i . '][mark]" id="mark' . $i . '"     value ="' . $StudentMark . '" placeholder="" min="0" max="' . $max . '" oninput="lengthConverter' . $i . '(this.value)" onchange="lengthConverter(this.value)" [. $i .] > /' . $max . '</td>';
        echo '<td>
      <input type="number" name="resultRecords[' . $i . '][convertedMark]" id="outputMeters' . $i . '" id="convertedMark' . $i . '" readonly value=' . $StudentConvertedMark . '></td>';
        echo '</tr>';
    ...
    ?>
        <script>
          function lengthConverter<?php echo $i; ?>(valNum) {
            console.log(valNum);
            var convertedMark = parseInt((valNum / <?php echo $max; ?>) * 100);
